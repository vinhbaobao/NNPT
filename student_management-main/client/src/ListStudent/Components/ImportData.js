...
class ImportData extends Component {
  state = {  
  // Ban đầu không có file nào được chọn
    selectedFile: null,
  };

  // Trên file chọn (từ cửa sổ bật lên)
  onFileChange = (event) => {
    // Update the state
    this.setState({ selectedFile: event.target.files[0] });
  };

  // Khi tải lên tệp (nhấp vào nút tải lên)
  onFileUpload = () => {
    // Tạo một đối tượng của formData
    const formData = new FormData(); 
// Cập nhật đối tượng formData
    formData.append(
      "myFile",
